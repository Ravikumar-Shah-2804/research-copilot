...
class RAGTracer:
    ...
    @contextmanager
    def trace_request(self, user_id: str, query: str, organization_id: Optional[str] = None):
        """Main request trace context manager."""
        with self.tracer.trace_rag_request(
            query=query, user_id=user_id, session_id=f"session_{user_id}", metadata={"simplified_tracing": True}, organization_id=organization_id
        ) as trace:
            try:
                yield trace
            finally:
